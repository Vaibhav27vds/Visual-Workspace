...
export const deploy = new Command("deploy").description("Deploy to a Convex deployment").option("-v, --verbose", "Show full listing of changes").option(
  "--dry-run",
...
).addOption(new Option("--debug-bundle-path <path>").hideHelp()).addOption(new Option("--debug").hideHelp()).addOption(new Option("--admin-key <adminKey>").hideHelp()).addOption(new Option("--url <url>").hideHelp()).addOption(new Option("--log-deployment-name").hideHelp()).showHelpAfterError().action(
  async (cmdOptions) => {
    const ctx = oneoffContext;
    const configuredDeployKey = readConfiguredAdminKey(cmdOptions.adminKey) ?? null;
    if (cmdOptions.checkBuildEnvironment === "enable" && isNonProdBuildEnvironment() && configuredDeployKey?.startsWith("prod:")) {
      logError(
        ctx,
...
async function handleProduction(ctx, options) {
  const { adminKey, url, deploymentNames } = await fetchProdDeploymentCredentials(ctx, options);
