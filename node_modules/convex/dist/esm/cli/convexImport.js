...
export const convexImport = new Command("import").description(
...
).addOption(new Option("--url <url>").hideHelp()).addOption(new Option("--admin-key <adminKey>").hideHelp()).argument("<path>", "Path to the input file").showHelpAfterError().action(async (filePath, options, command) => {
  ...
  const { adminKey, url: deploymentUrl } = await fetchDeploymentCredentialsProvisionProd(ctx, options);
  ...
  const urlName = tableName === null ? "" : `&tableName=${encodeURIComponent(tableName)}`;
