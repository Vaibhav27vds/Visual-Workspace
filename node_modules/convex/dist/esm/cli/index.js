...
async function main() {
  ...
  program.name("convex").usage("<command> [options]").addCommand(login, { hidden: true }).addCommand(init, { hidden: true }).addCommand(reinit, { hidden: true }).addCommand(dev).addCommand(deploy).addCommand(deployments, { hidden: true }).addCommand(run).addCommand(typecheck, { hidden: true }).addCommand(auth, { hidden: true }).addCommand(convexImport).addCommand(codegen).addCommand(dashboard).addCommand(docs).addCommand(update).addCommand(logs).addCommand(logout).addHelpCommand("help <command>", "Show help for given <command>").version(version).configureHelp({ visibleOptions: () => [] }).showHelpAfterError();
  try {
